...
class GradeExternalActivityXBlock(XBlock, StudioEditableXBlockMixin):
    ...
    usernames = (
        {}
    )  #: A dict used to avoid multiple calls to db {'username' : models.User , ...}.

    max_assessment_file_lines = (
        1000  #: The limit of csv lines in the uploded assessment file.
    )
    ...
    def student_view(self, context=None):
        ...
        gea_assessment = GeaAssessment(
            User.objects.get(id=self.xmodule_runtime.user_id), self
        )
        frag = Fragment(
            loader.render_template(
                "templates/edx_gea/student.html",
                {"score": gea_assessment.score, "comment": gea_assessment.comment},
            )
        )
        return frag
    ...
    def staff_view(self):
        ...
        frag = Fragment(
            loader.render_template(
                ...
                {
                    "upload_assessment_file_form": UploadAssessmentFileForm(
                        auto_id=True, initial={"csv_delimiter": get_default_delimiter()}
                    ),
    ...
    def upload_assessments(self, request, context=None):
        ...
        form = UploadAssessmentFileForm(
            request.POST,
            files={"assessment_file": uploaded_file},
            auto_id=True,
            gea_xblock=self,
        )
        ...
        if form.is_valid():
            ...
            return Response(
                u"<p class='gea_success'>{}</p>".format(
                    _("Thank you, the students assessment has been done.")
                )
            )
        else:
            return Response(
                loader.render_template(
                    "templates/edx_gea/form_errors.html",
    ...
    def handle_assessment_file(self, file):
        ...
        """
        assessment_file = csv.DictReader(
            file, ["username", "score", "comment"], delimiter=str(self.csv_delimiter)
        )
