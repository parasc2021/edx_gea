...
class GradeExternalActivityXBlockTests(ModuleStoreTestCase):
    def setUp(self):
        ...
        self.generate_modules_tree(
            self.course, "chapter", "sequential", "vertical", "edx_gea"
        )
    ...
    def test_handle_assessment_file(self):
        ...
        file = self.create_csv(
            [users[0], "5", u"Well done."], [users[1], "1", u"Very bad."]
        )
    ...
    def generate_form(self, raw_text):
        return UploadAssessmentFileForm(
            {"csv_delimiter": ","},
    ...
    def test_user_does_not_exist(self):
        CourseEnrollmentFactory.create(
            course_id=self.course.id, user=UserFactory(username="Joe")
        )
    ...
    def test_edx_grade_with_no_score_but_problem_loaded(self):
        ...
        CourseEnrollmentFactory.create(course_id=self.course.id, user=user)
        StudentModuleFactory.create(
            student=user,
            course_id=self.course.id,
            module_state_key=str(self.gea_xblock.location),
        )
