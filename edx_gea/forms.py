...
class UploadAssessmentFileForm(forms.Form):
    assessment_file = forms.FileField(
        label=ugettext_lazy("Assessment file"), validators=[validate_file_extension]
    )
    csv_delimiter = forms.ChoiceField(
        label=ugettext_lazy("CSV delimiter"),
        choices=((";", ugettext_lazy("Semicolon")), (",", ugettext_lazy("Comma"))),
    )
    ...
    def clean_assessment_file(self):
        assessment_file = csv.DictReader(
            self.cleaned_data["assessment_file"],
        ...
        for line, row in enumerate(assessment_file):
            if line > self.gea_xblock.max_assessment_file_lines:
                self.add_form_error(
                    line,
                    _(u"The csv file has more than %s lines.")
                    % self.gea_xblock.max_assessment_file_lines,
                )
                return
    ...
    def check_user_enrollment(self, line, username):
        if not CourseEnrollment.is_enrolled(
            self.gea_xblock.usernames[username], self.gea_xblock.course_id
        ):
            self.add_form_error(
                line, _(u"User %s is not enrolled to this course.") % username
            )
