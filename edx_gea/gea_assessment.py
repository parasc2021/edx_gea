...
class GeaAssessment(object):
    ...
    def __init__(self, user, gea_xblock):
        self.submission_id = {
            "item_id": gea_xblock.location,
            "item_type": "gea",
            "course_id": gea_xblock.course_id,
            "student_id": anonymous_id_for_user(user, gea_xblock.course_id),
        }
    ...
    def score(self, score):
        submissions_api.set_score(
            self.submission["uuid"], score.points_earned, score.points_possible
        )
    ...
    def comment(self, comment):
        self.submission = submissions_api.create_submission(
            self.submission_id, {"comment": comment}
        )
